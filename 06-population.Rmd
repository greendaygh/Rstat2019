...
R에서는 ```sample``` 함수를 사용해서 샘플링 작업을 수행할 수 있습니다. 
...
**[EXERCISE]** 3개의 빨간색 공과 2개의 녹색 공이 있는 주머니에서 반복을 허용해서 2개의 공을 뽑을 경우의 확률변수 $X$를 녹색 공의 갯수라 하자, $X$의 기댓값과 반복을 허용하지 않고 2개를 뽑을 경우의 기댓값을 구하시오.  

```{r eval=F}

```

또한 위 상황을 시뮬레이션하는 코드를 작성하고 위에서 계산한 기댓값과 같은 값이 나오는지 검증하시오. 
...
**[EXERCISE]** 위 예제에서 임의 복원 추출 방법으로 두 개의 표본을 추출하는 경우를 생각해보자. 이 경우 확률변수  $X$의 기댓값과 분산을 구하시오
...
**[EXERCISE]** 다음 파라미터를 갖는 uniform, normal, poisson, expoential, chi-squared 분포에서 임의로 1000개의 표본을 추출한 후 dataset을 만들고 데이터들의 분포를 히스토그램을 이용하여 확인하시오 
