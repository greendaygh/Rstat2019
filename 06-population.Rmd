...
위와 같은 확률 변수를 사용하는 목적을 우리의 실험을 예를 들어 다시 한번 생각해 보면 실험을 해서 얻는 데이터가 우리가 모르는 모집단으로부터 나오는 것이라 가정하고 최대한 많은 데이터를 확보한 후 확률변수를 정의하고 그 분포가 모집단의  분포와 같을 것으로 기대하여 기댓값과 분산을 계산하는 것 입니다. 앞에서 배운바와 같이 이러한 확률변수의 기댓값과 분산이 모집단의 대표값들 입니다. 


따라서 모집단에서 샘플링을 하는 방법이 중요하며 이 때 사용하는 방법이 Random sampling 입니다. $X_1, X_2, X_3, ..., X_n$을 모집단에서 뽑은 일련의 확률 변수라 한다면 이들의 분포가 같을 때 identically distributed 되었다고 하며 하나의 확률 변수의 값이 다른 변수들의 분포에 영향을 주지 않을 때 (즉 하나의 확률변수의 값을 알 때 다른 변수들의 분포가 변하지 않을 때) 그 변수는 다른 변수와 independent 하다고 합니다. 이 두 가지 특성을 갖는 샘플들을 indepentent and identically distributed 라고 해서 i.i.d.라 부르고 이는 랜덤 샘플링을 할 때 기본이 되는 개념입니다. 

예로 하나의 동전을 $n$번 던질 때 $X_i$를 i번째 던지는 동전의 앞 또는 뒷면이라 하면 $X_1, X_2, ..., X_n$은 i.i.d. 입니다. 주머니 공 꺼내기 예제에서 공을 다시 넣지 않고 꺼내는 경우는? 

R에서는 ```sample``` 함수를 사용해서 이러한 작업을 수행할 수 있습니다. 
...
**[EXERCISE]** 하나의 주사위를 10번 굴릴 경우 코드를 작성하시오 

**[EXERCISE]** 5개의 빨간색 공과 3개의 녹색 공이 있는 주머니에서 반복을 허용해서 3개의 공을 뽑을 경우의 기댓값과 반복을 허용하지 않고 3개를 뽑을 경우의 기댓값을 구하고 그 코드를 작성하시오 
